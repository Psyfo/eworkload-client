...
export class EditAcademicAdministrationComponent implements OnInit {
  ...
  getQualifications() {
    this.qualificationService
      ...
      .subscribe(
        ({ data }) => {
          this.qualifications = data.qualifications.map((qualification) => {
      ...
      .subscribe(
        (result) => {
          ...
          this.academicAdministrationService
            ...
            .subscribe(
              (result) => {
                this.activityModel = result.data.academicAdministrationActivity;
                ...
                this.selectedTitle = this.titles.find(
                  (title) => title.label === this.activityModel.title
                );
              },
              (err) => {
                console.error(err);
  ...
  onSubmit() {
    ...
    if (this.selectedTitle.value === '0' || this.selectedTitle.value === '1') {
